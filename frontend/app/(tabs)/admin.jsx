...
import { quizAPI, contestAPI, trackerAPI, authAPI } from '../../utils/api';
...
const AddItemModal = ({ visible, onClose, onSubmit, itemType, editingItem = null }) => {
    ...
    const renderFormFields = () => {
        if (itemType === 'quiz') {
            // Parse choices to show dropdown
            const choicesList = formData.choices 
                ? formData.choices.split(',').map(c => c.trim()).filter(c => c)
                : [];
            ...
            return (
                <>
                    ...
                    {choicesList.length > 0 ? (
                            ...
                            {choicesList.map((choice, index) => (
                                <TouchableOpacity
                                    ...
                                    style={[
                                        ...
                                        formData.answer === choice && styles.dropdownOptionSelected
                                    ]}
                                    onPress={() => setFormData({ ...formData, answer: choice })}
                                >
                                    <Text style={[
                                        styles.dropdownOptionText,
                                        formData.answer === choice && styles.dropdownOptionTextSelected
                                    ]}>
                                        {choice}
                                    </Text>
                        ...
                        <Text style={styles.helperText}>Enter choices above to select the correct answer</Text>
                    )}
                </>
            );
        } else if (itemType === 'contest') {
...
export default function Admin() {
    ...
    const loadAllData = async () => {
        try {
            ...
            // Parse requirements for contests if they're JSON strings
            const parsedContests = (contestsRes.contests || []).map(contest => ({
                ...contest,
